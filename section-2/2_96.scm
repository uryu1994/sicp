...
(define (install-polynomial-package)
  ...
  (define (reduce-coeff a)
    (let ((coeff-a (map coeff a)))
      (let ((gcd-a (gcd-list coeff-a)))
        (define (iter reduced rest)
          (if (null? rest)
              reduced
              (iter (adjoin-term (make-term (order (first-term rest))
                                            (div (coeff (first-term rest)) gcd-a))
                                 reduced)
                    (cdr rest))))
        (reverse (iter '() a)))))
  ...
  (define (pseudoremainder-terms a b)
    (let ((O1 (order (first-term a)))
          ...
          (C1 (coeff (first-term b))))
      (let ((factor (list (make-term 0 (exp C1 (add 1 (sub O1 O2)))))))
        (cadr (div-terms (mul-terms a factor) b)))))
  ...
  (trace gcd-terms pseudoremainder-terms)
